...
# FILL THIS IN
# initialize a graph for traversal process
traversalGraph = {}
...
# function to determine opposite direction
def opposite_direction(fowardDirection):
    dictionary = {
        "n": "s",
        "e": "w",
        "s": "n",
    ...
    return dictionary[fowardDirection]

# bfs for obtaining next direction
def bfs(traversalGraph, currentRoomId):
    visited = set()
    queue = []
    queue.append([currentRoomId])

    while queue:
        print(f"queue 50: {queue}")
        print(f"visited 51: {visited}")
        path = queue.pop(0)
        v = path[-1]
        print(f"path 54: {path}")
        print(f"v 55: {v}")
        if v not in visited:
            visited.add(v)
            print(f"traversalGraph[v] 58: {traversalGraph[v]}")
            for direct in traversalGraph[v]:
                print(f"direct 60: {direct}")
                # if a direction is unknown return that path
                if traversalGraph[v][direct] == '?':
                    return path
            #if all directions are known update queue until a path contains an unknown
            for direct in traversalGraph[v]:
                option_room = traversalGraph[v][direct]
                updated_path = path.copy()
                updated_path.append(option_room)
                queue.append(updated_path)
    # return none when queue is empty, when all verticies on queue are known  
    return None

# for testing while loop
test = True
x = len(roomGraph)
# x = 3

while x > len(traversalGraph) and test == True:
    # get current room id or room number
    currentRoomId = player.currentRoom.id
    # check if currentRoom is in my new graph if not add it to graph as a set containing all possible exits with val == ?
    if currentRoomId not in traversalGraph:
        traversalGraph[currentRoomId] = {i: "?" for i in player.currentRoom.getExits()}

    print(f"currentRoomId 82: {currentRoomId}")
    print(f"traversalGraph 83: {traversalGraph}")

    # Initialize next direction as None
    next_direction = None
    for direction in traversalGraph[currentRoomId]:
        print(f"direction line 88: {direction}")
        print(f"traversalGraph[currentRoomId]: {traversalGraph[currentRoomId]}")
        print(f"traversalGraph[currentRoomId][direction]: {traversalGraph[currentRoomId][direction]}")
        # loop through graph key values and if value contains "?" continue down that route otherwise continue to next value
        if traversalGraph[currentRoomId][direction] == '?':
            next_direction = direction
            print(f"next_direction 94: {next_direction}")
            # if next_direction value is not none move to that room and add direction to traversal path list
            if next_direction is not None:
                traversalPath.append(next_direction)
                player.travel(next_direction)
                next_room = player.currentRoom.id
                print(f"traversalPath 100: {traversalPath}")
                print(f"next_room/current room 101: {next_room}")
                print(f"traversalGraph 102: {traversalGraph}")
                # if next room or current room is not in my graph initialize it similar as before
                if next_room not in traversalGraph:
                    print(f"current room 105: {player.currentRoom.id}")
                    traversalGraph[next_room] = {i: '?' for i in player.currentRoom.getExits()}

            print(f"next_direction 108: {next_direction}")
            # Modify existing my-graph inorder to account for innerconnections between previous and current room
            traversalGraph[currentRoomId][next_direction] = next_room
            traversalGraph[next_room][opposite_direction(next_direction)] = currentRoomId 
            # set new current room for bfs
            currentRoomId = next_room
            print(f"traversalGraph 114: {traversalGraph}")
            print(f"current room 115 : {currentRoomId}") 
            break
    # test = False

    path = bfs(traversalGraph, currentRoomId)
    print(f'path 120:{path}')
    #path is none then traversal is completed for branch from starting room, otherwise the path is in reverse order via bfs
    if path is not None:
        for room in path:
            print(f"room 123:", room)
            print(f"currentRoomId 124:", currentRoomId)
            # travel backwards until you reach starting point
            for direction in traversalGraph[currentRoomId]:
                # if room in path is equal to direction follow that direction
                if traversalGraph[currentRoomId][direction] == room:
                    print("room is equal to direction")
                    print(f"traversalPath 128: {traversalPath}")
                    traversalPath.append(direction)
                    print(f"traversalPath 130: {traversalPath}")
                    player.travel(direction)
            currentRoomId = player.currentRoom.id
            print(f"currentRoomId 133", currentRoomId)
print(f"traversalPath 139", traversalPath)
print(f"traversalGraph 140", traversalGraph)
