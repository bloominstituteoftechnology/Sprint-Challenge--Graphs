...
def bfs(starting_node):
    ...
    while q.size() > 0:
        ...
        if v not in visited:
            ...
            for exit_direction in graph[v]:
                new_path = list(path)
                new_path.append(graph[v][exit_direction])
                q.enqueue(new_path)
    return None
...
while True:
    ...
    if len(unexploredExits) > 0:
        ...
        graph[previous_room_id][firstExit] = player.currentRoom.id

        graph[player.currentRoom.id][inverse_direction[firstExit]] = previous_room_id
