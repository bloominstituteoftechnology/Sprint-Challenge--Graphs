...
'''hold = []
    for directions in visited_rooms[start]:
        print('Directions', directions)
        hold.append(directions)
    currDirections.put(hold)
    currNode = start
    while currDirections.empty() is not True:
        print('-----------------------------------------')
        print('Visited Start', visited)
        print('Path Start', path)
        first = currDirections.get()
        print('First', first[-1])
        print('CurrNode', currNode)   
        nextNode = visited_rooms[currNode][first[-1]]
        print('NextNode',nextNode)
        if nextNode not in visited:
            path.append(first[-1])
            visited.append(nextNode)
            temp = visited_rooms[nextNode]
            print('temp', temp)
            for directions in temp:
                if temp[directions] == '?':
                    return path
                else:
                    tempPath = path + [directions]
                    print('TempPath', tempPath)
                    currDirections.put(directions)
...
##[296, |||||448, 378, 311,|||| 263, 217]
[475, 473, 470, 468, 420, 428, 331, 467, 318, 315, 261]
def helper(start):
    ...
    while nextItems.empty() is not True:
        ...
        if first[-1] not in visited:
            ##path.append(first[-1])         
            keys = visited_rooms[first[-1]]
            for directions in keys:
                tempPath = first + [keys[directions]]
                if keys[directions] == '?':
                    ##path = first
                    return tempPath[:-1]
...
while len(visited_rooms) != 500:
    ...
    if len(unexpExits) > 0:
        ...
        print(visited_rooms)
        path = helper(player.currentRoom.id)
        print('WOOOOO', path)
        path = path[1:]
        print('NEWPATH', path)
        for node in path:
            rooms = visited_rooms[player.currentRoom.id]
            print('ROOMS', rooms)
            print('NODE', node)
            newMove = None
            for directions in rooms:
                if rooms[directions] == node:
                    newMove = directions
            print('NEW MOVE', newMove)
            player.travel(newMove)
            traversalPath.append(inverseDirections[newMove])
            
        ##traversalPath.append(inverseDirections[prevDir])
