...
# start at room 0 and go in a final direction
#do dft which means keep going in that direction until you cant anymore
#but as you go through each room mark as visited
# also add corresponding directions
# once get as far in that one direction go back to nearest room with an unexplored exit
#then go in random direction again as far as possible and repeat prev steps
#somewhere add '?' to rooms that have other available exits but have not been explored
# do that until no more '?'
...
#         visited[player.currentRoom.id] = player.currentRoom.getExits()
#         prev_room_dir = inversePath[-1]
#         visited[player.currentRoom.id].remove(prev_room_dir)

#     while len(visited[player.currentRoom.id]) == 0:
#         switch_dir = inversePath.pop()
#         traversalPath.append(switch_dir)
#         player.travel(switch_dir)

#         move = visited[player.currentRoom.id].pop(0)
#         traversalPath.append(move)
#         inversePath.append(switched_dir[move])
#         player.travel(move)
...
#                 # if roomGraph[0][n] and 0 in rooms
