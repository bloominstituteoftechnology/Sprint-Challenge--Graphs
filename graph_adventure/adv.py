...
def travel():
    ...
    while True:
        ...
        if len(rooms) > 499:
            break
        if direction == None:
            bfs_path(current_room, path_to_take)
            break
        else:
            print(f'The direction is {direction}')

            # gets opposite direction for current direction
            opposite_direction = opposites[direction]

            # get next room
            next_room = roomGraph[current_room][1][direction]

            # update the current exits point to the room connected to the direction
            exits[current_room][direction] = next_room
            ...
            exits[current_room][opposite_direction] = previous_room
