...
exit_room = player.currentRoom.getExits()
        ...
        while q.size() > 0:
            ...
            if v not in visited:
                ...
                for next_vert in graph[v]:
                    # print(next_vert)
                    new_path = list(path)
                    new_path.append(graph[v][next_vert])
                    q.enqueue(new_path)
        return None
...
def path_to_dir(path):
    ...
    direction = []
    for i in path[1:]:
        for exit in graph[curr_room]:
            if i == graph[curr_room][exit]:
                direction.append(exit)
    return direction


while True:
    unexplored_exit = []
    current_room = graph[player.currentRoom.id]
    for room in current_room:
        if current_room[room] == '?':
            unexplored_exit.append(room)


    if len(unexplored_exit) > 0:
        travel_path = unexplored_exit[0]
        traversalPath.append(travel_path)
        prev_room = player.currentRoom.id
        player.travel(travel_path)
        ...
        graph[prev_room][travel_path] = player.currentRoom.id
        graph[player.currentRoom.id][inverse_direction[travel_path]] = prev_room
    else:
        back_track = bfs_path(player.currentRoom.id)
        if back_track is None:
            break
        for direction in path_to_dir(back_track):
