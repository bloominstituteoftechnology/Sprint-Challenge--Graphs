...
# def dft(self, starting_vertex, traversalGraph):
#     if traversalGraph is None:
#         visited = [False] * (len(self.vertices) + 1)
#
#     traversalGraph[starting_vertex] = True
#
#     for i in self.vertices[starting_vertex]:
#         if traversalGraph[i] is False:
#             self.dft(i, traversalGraph)
#
...
# traversalPath = dft(traversalGraph)
traversal_path = []

player.currentRoom = world.startingRoom

def establish_exits (currentRoom):
    exits = []
...
room_exits = establish_exits(player.currentRoom)

visited_rooms[player.currentRoom.id] = room_exits



def traverse (room_id, heading=None):
    rooms = visited_rooms[room_id]
    go_back = False

    for ind in range(len(rooms)):
        for dir in rooms[ind]:
            if rooms[ind][dir] and rooms[ind][dir] == "?":
                ...
                if player.currentRoom.id not in visited_rooms:
                    ...
                    visited_rooms[player.currentRoom.id] = establish_exits(player.currentRoom)


                for i in range(len(visited_rooms[player.currentRoom.id])):
                    opp_dir = get_opposite_direction(dir)
                    if opp_dir in visited_rooms[player.currentRoom.id][i] and visited_rooms[player.currentRoom.id][i][opp_dir] == "?":
                        visited_rooms[player.currentRoom.id][i][get_opposite_direction(dir)] = room_id # set exit to where we came from to opposite direction


                traverse(player.currentRoom.id, heading)
            else:
                go_back = True
    if go_back is True:
        player.travel(get_opposite_direction(heading))


traverse(player.currentRoom.id)
print(visited_rooms)
...
for move in traversal_path:
    player.travel(move)
    # visited_rooms.add(player.currentRoom) <-- changed this to a dictionary. Just use the value in the algorithm above.
