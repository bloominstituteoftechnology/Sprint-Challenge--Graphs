...
def dft_recursive(self, starting_room, count, path=None, graph=None, ):
        """
        ...
        # old code
        # def dft_r(self, starting_room, path=path, graph=graph):
        # if visited is None:
        #     visited = set()
        ...
        # dft_r(self, starting_room)

        def dft_r(starting_room, count=count, path=path, graph=graph, coming_from_direction=None):

            # try to see if the graph already has this room id
            if graph is None:
                graph = {}
                path = []
                adjacency_dict = {}
                for direction in starting_room.getExits():
                    adjacency_dict[direction] = '?'
                graph[starting_room.id] = adjacency_dict
            else:
                try: 
                    if graph[player.currentRoom.id] != None:
                        pass
            ...
            #if coming_from_direction has a value
            #set the appropriate value in the graph
            if coming_from_direction is not None:
                # get the previous room
                prevRoom = starting_room.getRoomInDirection(coming_from_direction)
                # update the graph so that the graph now show that the previous room is associated
                # with the coming from direction
                graph[starting_room.id][coming_from_direction] = prevRoom.id

            noQuestionMarks = False

            while not noQuestionMarks:
                # sorted_directions = list()
                # random.shuffle(sorted_directions)
                for direction, id in graph[starting_room.id].items():
                    if id != '?':
                        continue
                    
                    elif direction == 'w':
                        # get the value of the getRoomInDirection('w')
                        direction = 'w'
                        nextRoom = starting_room.getRoomInDirection(direction)
                        graph[starting_room.id][direction] = nextRoom.id
                        path.append(direction)
                        player.travel(direction)
                        dft_r(nextRoom, count, path, graph, 'e')
                
                    elif direction == 'e':
                        # get the value of the getRoomInDirection('e')
                        direction = 'e'
                        nextRoom = starting_room.getRoomInDirection(direction)
                        graph[starting_room.id][direction] = nextRoom.id
                        path.append(direction)
                        player.travel(direction)
                        dft_r(nextRoom, count, path, graph, 'w')

                    elif direction == 's':
                        # get the value of the getRoomInDirection('n)
                        direction = 's'
                        nextRoom = starting_room.getRoomInDirection(direction)
                        graph[starting_room.id][direction] = nextRoom.id
                        path.append(direction)
                        player.travel(direction)
                        dft_r(nextRoom, count, path, graph, 'n')
                        
                    elif direction == 'n':
                        # get the value of the getRoomInDirection('s')
                        direction = 'n'
                        nextRoom = starting_room.getRoomInDirection(direction)
                        graph[starting_room.id][direction] = nextRoom.id
                        path.append(direction)
                        player.travel(direction)
                        dft_r(nextRoom, count, path, graph, 's')

                    
                    
                # the room is fully explored
                noQuestionMarks = True
                if coming_from_direction == None:
                    return path
                if len(graph) == count:
                    # print(f'the Graph count is {len(graph)} and the count is {count} and the path count is {len(path)}')
                    return path
                
                path.append(coming_from_direction)
                player.travel(coming_from_direction)   
                        
        return dft_r(self, starting_room, count)
...
traversalPath = []
graph = {}
traversalPath = dft_recursive(player.currentRoom, len(roomGraph), traversalPath, graph)
