...
# Bring in Queue and Stack classes, probably gonna be useful.
...
class Graph:
    ...
    def add_edge(self, move):
        ...
        # Let's us look back at the last room
    ...
    def dft(self, vertex):
        ...
        print("LAST UNKNOWN", self.lastUnknown)
        i = 0
        ...
        for exits in player.currentRoom.getExits():
            currentExits.update({exits: '?'})
            self.lastUnknown[vertex] = currentExits
        for exit in self.vertices[vertex]:
            print(f"EXIT {i} FOR CURRENT ROOM {vertex}: ", exit)
    ...
    #     q = Queue()
    #     visited = set()
    #     q.enqueue([player.currentRoom.id])    if player.currentRoom.id not in graph.vertices:
