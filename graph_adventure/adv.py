...
[] You know YOU ARE DONE WHEN you have exactly 500 entries (0-499) 
in your graph AND NO '?' in the adjacency dictionaries. 
[] To do this, you will need to write a traversal algorithm 
that logs the path into traversalPath as it walks.
...
You can find the path to the shortest unexplored room by using a BREATH-FIRST SEARCH  <---- 
...
[]2. BFS will return the path as a list of room IDs. 
...
while len(roomPath) < len(roomGraph)-1:
    ...
    while len(roomPath[player.currentRoom.id]) == 0: # (all room exits explored)
        ...
        # Move toward new direction/exit
