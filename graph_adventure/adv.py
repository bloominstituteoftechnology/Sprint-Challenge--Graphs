...
while len(visited_dft) < len(roomGraph):
    ...
    if len(unVisited) > 0:
        ...
        # add new room to visited_dft if it's not there:
        if new_room not in visited_dft:
            # use getExits from player
        ...
        #kinda recursive call to bfs with new data
        directions = bfs(visited_dft, room)
...
#shortestPath(roomGraph,0,2)
#g = nx.Graph(roomGraph)
