...
def bfs(dictionary, room):
    ...
    while q.empty() is False:
        # p will be a list of room IDs, without directions
        p = q.get()
        # pull the first index off that list
        v = p[0]
...
# instantiate visited dictionary
visited = {}
count = 0
# start DFT
while len(visited) < len(roomGraph):

    # dictionary entry is: roomNum: {dir: roomNum, dir: roomNum..} with? for unknown rooms.

    # get current roomNum
    room = player.currentRoom.id

    # create room entry and add exits if the room isn't in visited
    if room not in visited:
        visited[room] = {
            direction: '?' for direction in player.currentRoom.getExits()}

    # check for unexplored exits
    unex = [direction for direction in visited[room]
            if visited[room][direction] == '?']
    print("UNEX", unex)
    # if there are unexplored rooms, choose a random one and go that direction:

    if len(unex) > 0:
        # generate a random number btwn 0 and end of list, travel in the direction stored at that index
        direction = unex[(random.randint(0, len(unex)-1))]
        player.travel(direction)

        # update traversal path with that direction
        ...
        # add new room to visited if it's not there:
        if new_room not in visited:
            visited[new_room] = {
                direction: '?' for direction in player.currentRoom.getExits()}
        # update new_room entry with old room:
        op_dir = opposite_direction(direction)
        visited[new_room][op_dir] = room
