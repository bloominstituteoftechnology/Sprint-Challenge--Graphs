...
traversalPath = [] # starts empty, needs to go to every room
...
mazeGraph = {}
path = [] # holds an array of our traversal path room IDs
            # convert this path to directions later on with our maze graph
advances = [] # holds an array of tuples of our player movements ('n','s'),'e','w')

prevRoom = [] # Store an Array of tuples to update roomExits for roomID
...
def traverseMaze(player, world):
    ...
    while len(mazeGraph) < len(world.rooms):
        # Add unexplored rooms to mazeGraph

        # DFT
        # Move player then update mazeGraph with roomIDs
        #while hasUnexploredRooms(player, mazeGraph):
            s = Stack()
            s.push(player.currentRoom.id)
            #prevroom [ (direction, roomID )] direction = [-1][0] id = [-1][1]
            while s.size() > 0 and len(mazeGraph) < len(world.rooms):
                print(len(mazeGraph))
                #clear bfs visited
                bfsVisited.clear()

                #current = s.pop()
                #if current not in mazeGraph:
                roomExits = {}
                roomExits.clear()
                possibleExits = []
                possibleExits.clear()
                for exit in player.currentRoom.getExits():
                    roomExits[exit] = '?'
                    #print("fill room exits with ? " + str(roomExits))
                if len(prevRoom) > 0:
                   # print("Maze Graph currently: " + str(mazeGraph))
                    #roomExits[prevRoom[-1][0]] = prevRoom[-1][1]
                    # Setting the previous room's direction pointer to current room ID
                    # mazeGraph[prevRoom[-1][1]][prevRoom[-1][0]] = player.currentRoom.id
                    # roomsToUpdate = mazeGraph[prevRoom[-1][1]]
                    # roomsToUpdate[prevRoom[-1][0]] = player.currentRoom.id
                    roomExits[getOppositeDirection(prevRoom[-1][0])] = prevRoom[-1][1]
                    #print("Maze Graph updated: " + str(mazeGraph))
                    #print("Room Exits updated" + str(roomExits))
                for exit in roomExits:
                    #print("Checking possible exits: " + str(roomExits))
                    if roomExits[exit] == '?':
                        possibleExits.append(exit)
                # update mazegraph with roomExits
                mazeGraph[player.currentRoom.id] = roomExits
                if len(possibleExits) > 0: 
                    randomDirection = getRandomDirection(possibleExits)
                    #print("Moving: " + str(randomDirection))
                    #print(mazeGraph)
                    #print("DFT possible exits:" + str(possibleExits) + "current: " + str(player.currentRoom.id))
                    #print("Chosen exit: " + str(randomDirection))
                    prevRoom.append((randomDirection, player.currentRoom.id))
                    #print("Prev Room List: " + str(prevRoom))
                    currentID = player.currentRoom.id
                    player.travel(randomDirection)
                    updateDirectionCount(randomDirection)
                    #print("DFS Player traveled to: " + str(player.currentRoom.id))
                    mazeGraph[currentID][randomDirection] = player.currentRoom.id
                    #print("Player's current room: " + str(player.currentRoom.id))
                    # add to stack
                    s.push(player.currentRoom.id)
                
                if len(possibleExits) == 0:
                    roomToExplore = findUnexploredRoom(player, world, roomExits)
                    s.push(roomToExplore)
                # call BFS and return appropriate currentRoom.ID to push onto DFT stack


                # PLAYER HAS HIT DEADEND, BEGIN BFS FOR FIRST PAST ROOM WITH UNEXPLORED DIRECTION
                # ONCE DIRECTION HAS BEEN FOUND ADD THE ID TO THE DFS STACK SO THE LOOP BEGINS AGAIN
...
def updateDirectionCount(direction):
    if direction == 'n':
        directionTakenCount[0] += 1
    if direction == 's':
        directionTakenCount[1] += 1
    if direction == 'e':
        directionTakenCount[2] += 1
    if direction == 'w':
        directionTakenCount[3] += 1

def getDirectionCount(direction):
    if direction == 'n':
        index = 0
    if direction == 's':
        index = 1
    ...
    if direction == 'w':
        index = 3
    return directionTakenCount[index]

def findUnexploredRoom(player, world, roomExits):
    q = Queue()
    # bfsVisited() up top!
    q.enqueue(player.currentRoom.id)
    #print("******----------------STARTING BFS -------------------*******")
    mazeGraph[player.currentRoom.id] = roomExits
    #print("BFS MAZE GRAPH START: " + str(mazeGraph))
    while q.size() > 0:
        current = q.dequeue()
        bfsPossibleExits = []
        bfsPossibleExits.clear()
        bestPossibleExits = []
        bestPossibleExits.clear()
       # print("BFS VISITED: " + str(bfsVisited))
        if current not in bfsVisited:
            deepBFSVisited.add(current)
        ...
        if len(bfsPossibleExits) > 0 or len(bestPossibleExits) > 0:
           # print("BFS POSSIBLE EXITS: " + str(bfsPossibleExits))
            if len(bfsPossibleExits) > 0:
                randomDirection = getRandomDirection(bfsPossibleExits)
            if len(bestPossibleExits) > 0:
                randomDirection = getRandomDirection(bestPossibleExits)
            #print("Moving: " + str(randomDirection))
            prevRoom.append((randomDirection, current))
            #print("BFS Prev Room List: " + str(prevRoom))
            player.travel(randomDirection)
            updateDirectionCount(randomDirection)
            print("BFS Player traveled to: " + str(player.currentRoom.id))
            #print("Players current room: " + str(player.currentRoom.id))
            q.enqueue(player.currentRoom.id)

def hasUnexploredRooms(player, mazeGraph):
    unexploredAvailable = False
    rooms = mazeGraph[player.currentRoom.id]
    for room in rooms:
        if room == '?':
     #       print("Unexplored room: " + str(room))
            unexploredAvailable = True
    return unexploredAvailable

def getRandomDirection(roomExits):
    ...
    if len(roomExits) > 1:
        ...
        #directionIndex = counts.index(min(counts))
         random.shuffle(roomExits)
         directionIndex = random.randrange(0, len(roomExits))
    direction = roomExits[directionIndex]
        #for directionTuple in directionTakenCount:
        #    if directionTuple[0] == direction:
...
# def dft_recursive(self, start, world, player, visited):
#     # base case
#     visited.add(start):
#     print(start)
#     for child in player.currentRoom.getExits()
#     # BFS for first room already visited that has a '?' as a connection
