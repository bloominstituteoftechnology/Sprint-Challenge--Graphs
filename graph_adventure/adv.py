...
def backtrack_to_unexplored(starting_vertex_id):
    ...
    while q.size() > 0:
        path = q.dequeue()
        v = path[-1] # looking at last item which your current room id
        if v not in visited:
...
def path_to_directions(path):
    ...
    current_room = path[0]  #[1, 2, 4, 5]
    directions = [] 
    for room in path[1:]: # iterate through out room IDs 
        for exit in graph[current_room]: #exit dictionary for current room
            if room == graph[current_room][exit]: 
                ...
                directions.append(exit) # appending the key 
                # [n]
    return directions
...
while True: #dfs
    ...
    if len(unExploredExits) > 0: #picking one the unexplored walk
        ...
        # firstExit = random.choice(unExploredExits)
        firstExit = random.sample(unExploredExits, 1)[0] #sampling one thing out of the list
        ...
        if player.currentRoom.id not in graph:
            ...
            # new room/ current room {1: {"n": "?", "e": "?", "s": ?, "w": "?"}}
