...
# graph = {0: [(3, 5), {'n': 1}], 1: [(3, 6), {'s': 0, 'n': 2}], 2: [(3, 7), {'s': 1}]}
...
for rooms in roomGraph:
    myGraph[rooms] = {}
    for directions in roomGraph[rooms][1]:
        myGraph[rooms][directions] = '?'

current_room = player.currentRoom.id

# if current_room not in myGraph:
#     visited = {}

# for visit in player.currentRoom.getExits():
#     visited[visit] = '?'

# myGraph[current_room] = visited

# visited = myGraph[current_room]
...
def find_direction (current_room): 
  """
  return open direction or first open direction   
  """
  if 'n' in myGraph[current_room] and myGraph[current_room]['n'] == '?':
    return 'n' 
  elif 'w' in myGraph[current_room] and myGraph[current_room]['w'] == '?':
    return 'w'
  elif 's' in myGraph[current_room] and myGraph[current_room]['s'] == '?':
    return 's'
  elif 'e' in myGraph[current_room] and myGraph[current_room]['e'] == '?':
    return 'e'
  else: 
    return None
...
def travel():
    ...
    while len(rooms) < 500:
        direction = find_direction(current_room)
        ...
        if direction == None:
            ...
            print(f"What direction? {direction}")
            print("direction", direction)
            opposite_direction = inverse_direction[direction]
            print("oppisite", opposite_direction)
            next_room = roomGraph[current_room][1][direction]
            print("next", next_room)
            myGraph[current_room][direction] = next_room

            previous_room = current_room
            print("prev room", previous_room)
            current_room = next_room
            print("current room", current_room)
            myGraph[current_room][opposite_direction] = previous_room
            print("current rom", current_room)
