...
while len(explored_rooms) < len(roomGraph):
	...
	while len(direction) > 0:
		...
		if pop_direction in accepted_directions:
			...
			player.travel(pop_direction)

			# add direction to traversal path

			traversalPath.append(pop_direction)
		...
		if initial_room != player.currentRoom.id:

			explored_rooms.add(initial_room)

			for e in directions_graph[initial_room]:

				if directions_graph[initial_room][e] == "?":

					if initial_room in explored_rooms:
						explored_rooms.remove(initial_room)
				...
				if i != (len(bfs_path)-1):
					...
					for direction in directions_graph[current_room]:
						...
						if directions_graph[current_room][direction] == bfs_path[i+1]:
							...
							print(f"{bfs_path[i]} {direction} {player.currentRoom.id}")

			direction = [player.currentRoom.id]

		else:

			if bfs_room not in b_visited:

				b_visited.add(bfs_room)
